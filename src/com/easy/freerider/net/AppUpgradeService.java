...
/*
 * 用于下载apk并安装， 会用notification 提示下载进度
 * */
import java.io.File;
...
//                System.out.println("Send DOWNLOAD_SUCCESS");
            }
            mNotificationManager.cancel(mNotificationId);
        }
    };
        ...
        public void run() {
            ...
            if (Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)){
                ...
                if (destDir.exists() || destDir.mkdirs()) {
                    destFile = new File(destDir.getPath() +File.separator+ getFileName(mDownloadUrl));
                    if (destFile.exists() && destFile.isFile() && checkApkFile(destFile.getPath())) {
